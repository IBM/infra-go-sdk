...
func main() {
	...
	// Default to empty so we can safely validate it when NOT in list mode
	diskNamesStr := flag.String("disk-names", "", "Comma-separated list of physical volumes (e.g., 'hdisk3,hdisk4')")

	viosProfile := flag.String("vios-profile", "default_profile", "Name of the LPAR profile to overwrite")
	...
	listMode := flag.Bool("list", false, "Set to true to purely LIST the current physical volume mappings for the LPAR")
	...
	if *listMode {
		...
		for _, mapping := range mappings {
			if strings.HasSuffix(strings.ToLower(mapping.AssociatedLogicalPartition.Href), targetLparLower) {
				...
				diskName := strings.TrimSpace(mapping.Storage.PhysicalVolume.VolumeName)
				if diskName == "" {
					diskName = strings.TrimSpace(mapping.ServerAdapter.BackingDeviceName)
				}

				fmt.Printf("   💾 Disk Name:         %s\n", diskName)
				fmt.Printf("      - Server Adapter:  %s\n", mapping.ServerAdapter.AdapterName)
				fmt.Printf("      - Target Device:   %s\n", mapping.TargetDevice.PhysicalVolumeVirtualTargetDevice.TargetName)
				fmt.Printf("      - Client Slot:     %d\n", mapping.ClientAdapter.VirtualSlotNumber)
				fmt.Printf("      - LU Address:      %s\n", mapping.TargetDevice.PhysicalVolumeVirtualTargetDevice.LogicalUnitAddress)
				fmt.Println("-------------------------------------------------------------------------")
			}
		}

		if count == 0 {
			fmt.Printf("   ❌ No Physical Volume mappings found for LPAR '%s' on this VIOS.\n", *lparName)
		} else {
			fmt.Printf("✅ Found %d Physical Volume mapping(s).\n", count)
		}
		
		// Exit early for read-only mode
		return
	}
	...
	for _, mapping := range mappings {
		if strings.HasSuffix(strings.ToLower(mapping.AssociatedLogicalPartition.Href), targetLparLower) {
			...
			if diskName != "" {
				alreadyMappedDisks[strings.ToLower(diskName)] = true
			}
		}
	}
	...
	if *deleteMode {
		...
		// --- CREATE LOGIC ---
		fmt.Printf("[Validate] Verifying requested Physical Volumes exist on VIOS '%s'...\n", *viosName)
		...
		for _, line := range strings.Split(out, "\n") {
			...
			if len(fields) > 0 && strings.HasPrefix(fields[0], "hdisk") {
				availableDisks[strings.ToLower(fields[0])] = true
			}
		}
		...
		if len(missingDisks) > 0 {
			fmt.Printf("\n⚠️  Warning: The following Physical Volumes do NOT exist on VIOS '%s' and will be SKIPPED:\n", *viosName)
		...
		if len(validDisks) == 0 {
			log.Fatalf("\n❌ Cannot proceed: None of the requested Physical Volumes exist on the VIOS.")
		}

		fmt.Printf("\n✅ %d valid Physical Volume(s) found on VIOS.\n", len(validDisks))
	...
	// =========================================================================
	// 5. SAVE PROFILE (Only if changes were actually made)
	// =========================================================================
	if operationStatus == "SUCCESS" || operationStatus == "SUCCESS_WITH_RMC_WARNING" {
		fmt.Printf("\n[Profile] Saving running configuration to LPAR profile '%s'...\n", *viosProfile)
		...
		if saveErr != nil {
			...
			fmt.Println("✅ Success: LPAR profile saved. The changes will persist across reboots.")
		}
	} else {
		fmt.Printf("\n[Profile] No architectural changes were made to the LPAR. Profile save skipped.\n")
	}
