...
func (c *HmcRestClient) logRawTraffic(direction, url, payload string) {
	...
	safePayload := payload
	// Mask the session token if it exists in the payload
	if c.session != "" {
		safePayload = strings.ReplaceAll(safePayload, c.session, "********[REDACTED]********")
	}

	// Truncate massive payloads to keep the terminal readable
	if len(safePayload) > 50000 {
		safePayload = safePayload[:50000] + "\n... [TRUNCATED]"
	}

	c.Logger.Debug(fmt.Sprintf("HTTP %s", direction),
		"url", url,
		"payload", "\n"+safePayload,
	)
}